...
export async function getAssets(): Promise<AssetType[]> {
  ...
  if (mongoose.connection.readyState !== 1) {
    await mongoose.connect(process.env.MONGODB_URI as string); // به صورت مستقیم از Mongoose متصل شوید
  }
...
type NewAssetInput = {
  assetType: string;
  assetAmount: number;
  assetAmountType: string;
  description?: string;
};

export async function addAsset(data: NewAssetInput): Promise<boolean> {
  if (mongoose.connection.readyState !== 1) {
